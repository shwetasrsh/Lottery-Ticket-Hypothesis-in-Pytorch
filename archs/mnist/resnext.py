        ...
        #self.basic = nn.Sequential(OrderedDict(
         #   [
          #      ('conv1_0', nn.Conv2d(in_planes, inner_width, kernel_size=1, stride=1, bias=False)),
           ...
           #]
      # ))
        self.conv1 = nn.Conv2d(in_planes, inner_width, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(inner_width)
        self.conv2 = nn.Conv2d(inner_width, inner_width, kernel_size=3, stride=stride, padding=1, groups=cardinality, bias=False)
        self.bn2 = nn.BatchNorm2d(inner_width)
        self.conv3 = nn.Conv2d(inner_width, self.expansion*inner_width, kernel_size=1, bias=False)
        self.bn3 = nn.BatchNorm2d(self.expansion*inner_width)
        ...
        if stride != 1 or in_planes != inner_width*self.expansion:
            self.shortcut = nn.Sequential(
                ...
                nn.BatchNorm2d(self.expansion*inner_width)
            )
...
class resnext(nn.Module):
    def __init__(self, num_blocks, cardinality, bottleneck_width, expansion=2, num_classes=10):
        ...
        self.linear = nn.Linear(self.cardinality*self.bottleneck_width*8, num_classes)
...
def resnext26_4x32d():
    return resnext(num_blocks=[3,3,3], cardinality=4, bottleneck_width=32)


def resnext26_8x16d():
    return resnext(num_blocks=[3,3,3], cardinality=8, bottleneck_width=16)


def resnext26_16x8d():
    return resnext(num_blocks=[3,3,3], cardinality=16, bottleneck_width=8)
